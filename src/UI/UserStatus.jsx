    ...
    <div className="cc mt-8 mb-5">
      <div className="container bg-[#2b3945] p-2 rounded-sm">
        ...
        <div className="flex flex-col gap-2 mt-2 w-full overflow-y-auto h-[7.5rem] task_cont">
          {data.map((cur,index)=>{
            return <div className="border border-green-300 flex items-center justify-between py-1 px-3 rounded-sm text-md w-full" key={index}>
            <p className="capitalize">{cur.name.split(" ")[0]}</p>
            <p className="text-blue-300">{cur.dashboard.new}</p>
            <p className="text-yellow-300">{cur.dashboard.accept}</p>
            <p>{cur.dashboard.complete}</p>
            <p className="text-red-300">{cur.dashboard.failed}</p>
          </div>
          })}
        </div>
